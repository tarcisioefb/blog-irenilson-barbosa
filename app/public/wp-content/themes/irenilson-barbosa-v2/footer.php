...
<footer class="footer" role="contentinfo">
	<div class="wrap">
		<div class="footer__row">
			<div class="footer__logo"><?php $logo = ib_opt('site_logo'); if ($logo) : $logo_id = attachment_url_to_postid($logo); if ($logo_id) : echo wp_get_attachment_image($logo_id, 'medium', false, ['alt' => esc_attr(get_bloginfo('name')), 'style' => 'height:36px;width:auto', 'loading' => false]); else : ?><img src="<?php echo esc_url($logo); ?>" alt="<?php echo esc_attr(get_bloginfo('name')); ?>" style="height:36px;width:auto"><?php endif; else : ?><span style="font-family:var(--serif);font-weight:700;color:var(--ink)"><?php echo esc_html( get_bloginfo( 'name' ) ); ?></span><?php endif; ?></div>
...
<div id="ib-cookie-banner" role="alert" style="display:none;position:fixed;bottom:0;left:0;right:0;z-index:9999;background:var(--ink);color:#fff;padding:var(--space-4) var(--space-6);font-size:var(--text-13);line-height:1.5;box-shadow:0 -4px 20px rgba(0,0,0,.2)">
	<div style="max-width:var(--wrap);margin:0 auto;display:flex;flex-wrap:wrap;align-items:center;gap:var(--space-4)">
		<p style="flex:1;margin:0;min-width:200px">Usamos cookies do Google Analytics para entender como você navega. Ao continuar, você concorda com nossa <a href="/privacidade/" target="_blank" style="color:var(--tx-inv);text-decoration:underline">Política de Privacidade</a>.</p>
